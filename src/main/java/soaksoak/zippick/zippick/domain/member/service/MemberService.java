...
import soaksoak.zippick.zippick.global.utils.JwtUtil;
...
public class MemberService {
    ...
    public MemberLoginResponseDto requestRefresh(String refreshToken) {
        ...
        Claims claims = JwtUtil.parseToken(foundRefreshToken.getValue(), refreshKey);
    ...
    public String getUsernameFromToken(String accessToken) {
        ...
        String token = accessToken.split(" ")[1]; // authorization의 두 번째 단어
        // AccessToken에 들어있는 username 값 가져오기
        Claims claims = JwtUtil.parseToken(token, accessKey);
        String username = claims.get("username").toString();
        return username;
    }
}
